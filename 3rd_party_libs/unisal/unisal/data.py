...
class MemoryImageDataset(Dataset):
	...
	def __init__(self, images):
		...
		self.n_images_dict = {
			img_idx: 1 for img_idx in range(len(self.images))}
	...
	def preprocess_sequence(self, frame_seq, out_size):
		transformations = []
		transformations.append(transforms.ToPILImage())
		transformations.append(transforms.Resize(
			out_size, interpolation=PIL.Image.LANCZOS))
	...
	def get_all_data(self):
		out_size = self.out_size_dict[0]
		imgs = self.preprocess_sequence(self.images, out_size)
		return imgs
...
class MemoryFramesDataset(Dataset):
	...
	def __init__(self, images):
		...
		img_size = tuple(images[0].shape[:2])
		self.out_size = get_optimal_out_size(img_size)
	...
	def get_data(self, vid_nr, start):
		...
		frame_nrs = list(range(start, n_images + 1, self.frame_modulo))
		frame_nrs = list(range(vid_nr, min(vid_nr + self.frame_modulo, n_images)))
		
		if False:
			print('vid_nr', vid_nr)
			print('start', start)
			print('n_images', n_images)
			print('frame_nrs', frame_nrs)
			input('...')
